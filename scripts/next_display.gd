...
@export var piece_spacing: float = 78.0
...
func _draw() -> void:
	...
	var next_pieces: Array = game_logic.get_next_pieces(preview_count)
	for i in range(next_pieces.size()):
		_draw_piece(next_pieces[i], size.x / 2.0, header_height + top_padding + i * piece_spacing)
